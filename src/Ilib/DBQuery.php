...
class Ilib_DBQuery
{
    ...
    public function useStored($value = true)
    {
        if (!in_array($value, array(true, false))) {
            throw new Exception("Første parameter til DBQuery->useStored() er ikke ente true eller false");
        }
    ...
    public function getRecordset($fields, $use = "", $print = false)
    {
        ...
        if ($csql != "") {
            ...
            if (!$db->nextRecord()) {
                throw new Exception("Kunne ikke eksekvere SQL-sætning");
            }
    ...
    protected function getSqlString($extra_condition = "")
    {
        ...
        if ($this->error->isError()) {
            ...
            // @todo Den skal ikke trigger error. Det giver problemer hvis der er fejl i betaling af faktura. Men hvad skal den så? /Sune (29/6 2005)
